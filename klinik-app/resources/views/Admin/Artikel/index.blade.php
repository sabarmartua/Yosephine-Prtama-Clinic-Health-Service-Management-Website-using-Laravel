            ...
            <div class="content">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">{{ Str::limit($artikel->judul, 40) }}</h5>
                                @if($artikel->gambar)
                                <img src="{{ asset('storage/' . $artikel->gambar) }}" alt="{{ $artikel->judul }}" class="img-fluid">
                                @endif
                                <p class="card-text">Kategori: {{ $artikel->kategoriArtikel->nama }}</p>
                                <p class="card-text truncate">Isi: {{ Str::limit($artikel->isi, 100) }}</p>
                                <form action="{{ route('artikel.destroy', $artikel->id) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('artikel.edit', $artikel->id) }}" class="btn btn-info btn-sm">Edit</a>
                                    <button type="submit" class="btn btn-danger btn-sm delete-button">Hapus</button>
                                    <a href="{{ route('artikel.show', $artikel->id) }}" class="btn btn-warning btn-sm">Detail</a>
                                </form>
                            </div>
                        </div>
                    </div>
