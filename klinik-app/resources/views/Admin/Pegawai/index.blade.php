                    ...
                    <div class="col-md-4">
                        <div class="card mb-3">
                            <div class="row no-gutters">
                                <div class="col-md-6">
                                    @if($pegawai->gambar)
                                    <img src="{{ asset('storage/' . $pegawai->gambar) }}" class="card-img" alt="{{ $pegawai->nama }}">
                                    @endif
                                </div>
                                <div class="col-md-6">
                                    <div class="card-body">
                                        <h5 class="card-title">{{ $pegawai->nama }}</h5><br>
                                        <p class="card-text"><span>Profesi:</span> {{ $pegawai->profesi }}</p><br>
                                        <p class="card-text"><span>Nomor Telepon:</span> {{ $pegawai->nomor_telepon }}</p><br>
                                        <p class="card-text"><span>Tanggal Lahir:</span> {{ $pegawai->tanggal_lahir }}</p><br>
                                        <p class="card-text"><span>Alamat:</span> {{ $pegawai->alamat }}</p><br>
                                        <p class="card-text"><span>Pendidikan Terakhir:</span> {{ $pegawai->pendidikan_terakhir }}</p><br>
                                        <form action="{{ route('pegawai.destroy', $pegawai->id) }}" method="POST" class="delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{ route('pegawai.edit', $pegawai->id) }}" class="btn btn-info btn-sm">Edit</a>
